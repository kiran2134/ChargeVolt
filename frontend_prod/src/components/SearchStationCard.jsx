    ...
    return (
        <div className=" w-full flex-box p-5 gap-5 bg-violet-200  bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-violet-200  to-purple-200 rounded-2xl flex-col shadow-lg">
            <div className=" w-full flex-box justify-between">
                <div className="w-[65%] flex-box flex-col items-start gap-1">
                    <h1 className=" text-2xl font-bold">
                        {stationData.station_name}
                    </h1>
                    <div className=" flex-box gap-1 text-sm w-full items-start justify-start">
                        <MapPin className=" size-5" />
                        <h1 className=" capitalize truncate ">
                            {stationData.address.toLowerCase()}
                        </h1>
                    </div>
                    <GradientLink
                        url={stationData._id}
                ...
                <div className=" flex-box max-w-[35%] h-full items-start flex-wrap gap-2 text-md font-semibold justify-end">
                    {stationData.availslottype.map((slotName) => {
                        return (
                            <span className=" px-2 py-1  text-center rounded-md border-2 border-violet-600 ">
                                {slotName}
                            </span>
                        );
                    })}
